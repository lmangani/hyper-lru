...
import HyperLRU from '.';

const lruWithDuplicates = () => {
	const lru = new HyperLRU({maxSize: 2});
	lru.set('key', 'value');
	lru.set('keyDupe', 1);
	lru.set('keyDupe', 2);
	return lru;
...
test('main', t => {
	t.throws(() => {
		new HyperLRU(); // eslint-disable-line no-new
	}, /maxSize/);
});

test('.get() / .set()', t => {
	const lru = new HyperLRU({maxSize: 100});
	lru.set('foo', 1);
	lru.set('bar', 2);
	t.is(lru.get('foo'), 1);
	t.is(lru.size, 2);
...
test('max size', t => {
	const lru = new HyperLRU({maxSize: 3});
	lru.set('1', 1);
	lru.set('2', 2);
	lru.set('3', 3);
	t.is(lru.size, 3);
...
test('`onEviction` option method is called after `maxSize` is exceeded', t => {
	...
	const lru = new HyperLRU({maxSize: 1, onEviction});
...
test('resize omits evictions', t => {
	...
	const lru = new HyperLRU({maxSize: 2, onEviction});
